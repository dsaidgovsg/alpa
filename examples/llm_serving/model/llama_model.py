...
@flax.struct.dataclass
class LLaMALMOutput(ModelOutput):
...
def get_config(name, **kwargs):
    # TODO: fill in intermediate_size from params.json
    if name == "llama-7b":
        config = LLaMAConfig(
            hidden_size=4096, num_attention_heads=32, num_hidden_layers=32,
            intermediate_size=11008, max_sequence_length=2048, 
            use_cache=True, version=1,
        )
    elif name == "llama-13b":
        config = LLaMAConfig(
            hidden_size=5120, num_attention_heads=40, num_hidden_layers=40,
            intermediate_size=11008, max_sequence_length=2048, 
            use_cache=True, version=1,
        )
    elif name == "llama-33b":
        config = LLaMAConfig(
            hidden_size=6656, num_attention_heads=52, num_hidden_layers=60,
            intermediate_size=11008, max_sequence_length=2048, 
            use_cache=True, version=1,
        )
    elif name == "llama-65b":
        config = LLaMAConfig(
            hidden_size=8192, num_attention_heads=64, num_hidden_layers=80,
            intermediate_size=11008, max_sequence_length=2048, 
            use_cache=True, version=1,
        )
    else:
...
def init_model_aval(config):
    ...
    model = FlaxLLaMAForCausalLMModule(config, dtype=config.dtype)
